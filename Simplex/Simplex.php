...
class Simplex
{
    ...
    private function gerarMatriz($expr, array $restr = [])
    {
        ...
        for ($i = 0; $i < (count($folgas) + 1); $i++) {
            $matriz[] = array_fill(0, count($matriz[0]), (float) 0);
        }
        ...
        foreach ($valores as $k => $v) {
            ...
            $matriz[count($matriz) - 1][$k + 1] = $valor * -1;
        }
    ...
    {
        $slice = array_slice($matriz, -1)[0];

        $entrada = min(array_slice($slice, 1));
        $entradaIdx = array_search($entrada, $slice);

        $entradaCol = array_map(function($e) {
            if ($e < 0)
                return $e;
        }, array_slice($slice, 1));

        if ($this->condicaoParadaColuna($entradaCol)) {
    ...
    public function executar($expr, array $restr = [])
    {
        ...
        while ($this->condicaoParada(end(array_slice($matrizes, -1)[0]))) {
            if (is_null(end(end(array_slice($matrizes, -1)[0])))) {
                return $matrizes;
            }

            $voltas++;

            if ($voltas > Simplex::MAXVOLTAS) {
                return $matrizes;
            }

            $cp = $this->copiarMatriz(array_slice($matrizes, -1)[0]);

            $matrizes[] = $this->simplex($cp);
        }

        //$ultimaMatriz = $this->copiarMatriz(array_slice($matrizes, -1)[0]);
